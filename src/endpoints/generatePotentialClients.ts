...
const RequestBodySchema = z.object({
  businessDescription: z.string().openapi({
    ...
    example:
      "We are a software development agency specializing in building web applications for startups.",
  }),
});
...
export class GeneratePotentialClients extends OpenAPIRoute {
  schema = {
    ...
    summary:
      "Generate client keywords & find Google Places results for the first keyword", // Updated summary
    request: {
      body: {
        content: {
          "application/json": {
    ...
    responses: {
      "200": {
        description:
          "Returns Google Places API search results for the first potential client keyword generated by AI", // Updated description
        content: {
          "application/json": {
      ...
      "500": {
        description:
          "Internal Server Error (API key issues, AI error, Places API error, etc.)", // Updated description
        content: {
          "application/json": {
      ...
      "404": {
        // Added case where no keywords are generated
    ...
    try {
      ...
      const placesResponse = await fetch(placesUrl, {
        ...
        headers: {
          ...
          "X-Goog-FieldMask":
            "places.displayName,places.formattedAddress,places.priceLevel,places.internationalPhoneNumber,places.userRatingCount",
        },
      ...
      if (!placesResponse.ok) {
        ...
        console.error(
          `Places API Error (${placesResponse.status}): ${errorBody}`
        );
        c.status(500);
        return c.json({
          error: `Failed to fetch data from Google Places API. Status: ${placesResponse.status}`,
        });
      }

      const places = (await placesResponse.json()) as any;
      const mappedPlaces = places.places.map((place) => ({
      ...
      c.status(500);
      return c.json({
        error: "Failed to process request to Google Places API.",
      });
    }
  }
}
